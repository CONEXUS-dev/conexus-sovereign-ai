...
class SwayAgent:
    ...
    def process_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        ecp_result = self._collapse_ecp(task_input, gear_context, context, gear_state, domain, mirror_tier_key)
    ...
    def _collapse_ecp(
        ...
        """
        # Gear 1: Rapport — establish context
        if gear_state:
            gear_state.rapport_established = True
            gear_state.advance_gear("Rapport: Collapse context established")

        # Gear 2: Truth — accept gear context
        if gear_state:
            gear_state.truth_statement = gear_context or "collapse_processing"
            gear_state.advance_gear(f"Truth: {gear_state.truth_statement}")

        # Gear 3: Symbol — execution signal analysis + symbolic field injection
        ...
        if gear_state:
            ...
            gear_state.advance_gear(f"Symbol: domain={domain}, dominant={execution_signals.get('dominant_signal')}")

        # Gear 4: Contradiction — detect and RESOLVE (unlike Opie who holds)
        contradictions = self._detect_contradictions(task_input, domain=domain)
        resolved = self._resolve_contradictions(contradictions)
        if gear_state:
            gear_state.contradictions = [f"{c['pole_a']} ↔ {c['pole_b']}" for c in contradictions]
            gear_state.resolved_directives = [r.get("directive", "") for r in resolved]
            gear_state.advance_gear(f"Contradiction: {len(contradictions)} detected, {len(resolved)} resolved")

        # Gear 5: Hold/Resolve — in Collapse mode, we resolve
        if gear_state:
            gear_state.advance_gear(f"Resolve: {len(resolved)} directives extracted")

        # Gear 6-8: Mode Activation — LLM call in Collapse Mode
        ...
        if gear_state:
            ...
            gear_state.advance_gear("Roam: implementation space explored via LLM")

        # Parse structured output
        parsed = self._parse_collapse_output(llm_output)

        # Stress — extract breakthroughs (survival under pressure)
        breakthroughs = self._extract_breakthroughs(llm_output)
        if gear_state:
            gear_state.stress_results = f"{len(breakthroughs)} breakthroughs extracted"
            gear_state.stress_survived = True
            for bt in breakthroughs:
                gear_state.tag_breakthrough(str(bt))
            gear_state.advance_gear(f"Stress: {len(breakthroughs)} breakthroughs")

        # Ethics/Value — identify handoff items
        ...
        if gear_state:
            gear_state.seal_summary = f"Collapse complete: confidence={confidence:.2f}, {len(resolved)} resolved, {len(breakthroughs)} breakthroughs"
    ...
    def _parse_collapse_output(self, llm_output: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if current_section == "execution_steps" and stripped:
                # Skip sub-bullets and indented detail lines
                if stripped.startswith(("*", "-", "\t")) or line.startswith(("\t", "    ")):
                    continue
                # Only parse lines that look like numbered steps (N. or digit.)
                if not (stripped.startswith("N.") or (stripped[0].isdigit() and "." in stripped[:4])):
                    continue
                step_num += 1
                # Try to parse "N. [ACTION] | priority: X | effort: Y"
                parts = stripped.split("|")
                action = parts[0].strip().lstrip("0123456789.-) N")
                priority = "medium"
                effort = "unknown"
                for part in parts[1:]:
    ...
    def _assess_confidence(self, task_input: str, llm_output: str) -> float:
        ...
        upper_out = llm_output.upper()
        section_markers = [
            "EXECUTION STEPS", "MISSION COMPRESSION", "DEPENDENCIES",
            "RISKS", "BREAKTHROUGHS", "HANDOFF",
        ]
        sections_found = sum(1 for m in section_markers if m in upper_out)
        score += min(sections_found * 0.05, 0.2)

        # Breakthroughs and resolution signals
        if "[BREAKTHROUGH]" in llm_output:
            score += 0.08
        if "priority:" in llm_output.lower():
            score += 0.05

        # Output length tiers
        out_len = len(llm_output)
        if out_len > 1000:
            score += 0.1
        elif out_len > 500:
            score += 0.07

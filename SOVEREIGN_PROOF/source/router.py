...
Routes tasks to the appropriate agent (Sway or Opie) based on task content,
explicit assignment, or smart analysis of the input.
...
def _smart_route(task_input: str) -> str:
    ...
    if opie_score > sway_score:
        return "opie"
    # Default to sway (cheaper, more common)
    return "sway"
...
def format_routing_decision(
    ...
    return {
        ...
        "routing_method": (
            "explicit" if task_data.get("agent_assignment", "").lower() in ("sway", "opie", "both")
            else "smart"
        ),
